...
export function resolveSequence(clips, config) {
  if (!clips.length) return [];
  let groups = config;
  if (!groups) {
    const occupied = new Set();
    for (const clip of clips) for (const track of clip.tracks) {
      if (occupied.has(track.name)) throw Error('Animation clips overlap. Set the intended sequence in characters.js.');
      occupied.add(track.name);
    }
    groups = [{clips:clips.map(c => c.name)}];
  }
  const seen = new Set();
  ...
  if (seen.size !== clips.length) throw Error('Sequence omits animation clips. Review every clip before publishing.');
  return sequence;
}
function snapshot(model) {
  const values=[];
  model.traverse(n => values.push([n,n.position.clone(),n.quaternion.clone(),n.scale.clone(),n.morphTargetInfluences?.slice()]));
  return () => { for(const [n,p,q,s,m] of values) { n.position.copy(p);n.quaternion.copy(q);n.scale.copy(s);if(m)n.morphTargetInfluences.splice(0,m.length,...m); } };
}
export class SequencePlayer {
  constructor(model, clips, config) {
    this.model=model; this.groups=resolveSequence(clips,config);
    this.duration=this.groups.reduce((sum,g)=>sum+g.duration,0);
    this.mixer=new THREE.AnimationMixer(model);this.restore=snapshot(model);this.reset();
  }
  enter(index) {
    // Retain the outgoing final pose for properties not keyed by the next group.
    const retain=snapshot(this.model);this.mixer.stopAllAction();retain();this.index=index;
    for(const clip of this.groups[index]?.clips || []) {
      const action=this.mixer.clipAction(clip);action.reset();action.setLoop(THREE.LoopOnce,1);
      action.clampWhenFinished=true;action.play();
    }
    this.mixer.update(0);
  }
  reset() { this.mixer.stopAllAction();this.restore();this.time=0;this.localTime=0;this.enter(0); }
  update(dt) {
    ...
    while(remaining>1e-9 && this.groups[this.index]) {
      ...
      this.mixer.update(step);this.time+=step;this.localTime+=step;remaining-=step;
      if(this.localTime>=this.groups[this.index].duration-1e-8 && this.index+1<this.groups.length) {
        this.enter(this.index+1);this.localTime=0;
      } else if(step===0) break;
    }
  }
  seek(time) { this.reset();this.update(Math.min(time,this.duration)); }
  get complete() { return this.time>=this.duration-1e-7; }
}
...
export class CharacterRig {
  constructor(gltf, config) {
    ...
    this.player=new SequencePlayer(this.model,gltf.animations,config.sequence);
    const bounds=posedBounds(this.model), centre=bounds.getCenter(new THREE.Vector3());
    ...
    this.tracker=config.motionNode ? this.model.getObjectByName(config.motionNode) : this.model.getObjectByName('Frame') || this.model.getObjectByName('Root') || this.model;
    if(!this.tracker)throw Error(`Motion node not found: ${config.motionNode}`);
    this.origin=this.trackerPoint();this.compensation.position.copy(this.baseOffset);
    this.size.scale.setScalar(config.height/height);this.heading.rotation.y=THREE.MathUtils.degToRad(config.facingDegrees);
  ...
  compensate() {
    ...
    this.compensation.position.x-=delta.x;this.compensation.position.z-=delta.z;
  ...
  reset() { this.player.reset();this.compensate(); }
  update(dt) { this.player.update(dt);this.compensate(); }
  seek(t) { this.player.seek(t);this.compensate(); }
}
