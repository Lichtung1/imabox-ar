...
const characters = (() => {
  const window = {};
  new Function('window', fs.readFileSync(new URL('../characters.js', import.meta.url), 'utf8'))(
    window,
  );
  return window;
})();
const config = (id) => ({
  ...characters.IMABOX_DEFAULTS,
  ...characters.IMABOX_CHARACTERS.find((c) => c.id === id),
});
const clip = (name, target, values, duration = 2) =>
  new THREE.AnimationClip(name, duration, [
    new THREE.NumberKeyframeTrack(target, [0, duration], values),
  ]);
test('the ending finishes and holds after forward travel stops', () => {
  const model = new THREE.Group();
  const player = new SequencePlayer(model, [clip('full', '.position[y]', [0, 9], 9)], null);
  player.update(3);
  assert.equal(approachProgress(player.time, { start: 1, end: 3 }), 1);
  assert.equal(player.complete, false);
  player.update(6);
  assert.equal(player.complete, true);
  assert.equal(model.position.y, 9);
  player.update(20);
  assert.equal(model.position.y, 9);
  player.reset();
...
test('all disjoint object clips run together through the longest clip', () => {
  ...
  const player = new SequencePlayer(
    model,
    [clip('body', '.position[y]', [0, 2]), clip('wing', '.rotation[z]', [0, 4], 4)],
    null,
  );
  player.update(3);
  assert.equal(model.position.y, 2);
  assert.equal(player.complete, false);
  player.update(1);
  assert.equal(model.rotation.z, 4);
});
test('overlapping alternatives require explicit ordering and no clips may be silently omitted', () => {
  ...
  const model = new THREE.Group();
  const player = new SequencePlayer(model, clips, [{ clips: ['run'] }, { clips: ['end'] }]);
  player.update(5);
  assert.equal(player.duration, 5);
  assert.equal(model.position.x, 5);
  assert.equal(player.complete, true);
  player.reset();
  player.update(2.5);
  assert.ok(Math.abs(model.position.x - 2.5) < 1e-6);
});
test('a long frame cannot cross the stopping circle and latch can stop at boundary', () => {
  ...
  assert.ok(Math.abs(result.fraction - 0.3) < 1e-9);
...
test('real skinned model is grounded/scaled and root travel compensated, including replay/final pose', async () => {
  ...
  const b = fs.readFileSync(new URL('../char1.glb', import.meta.url));
  const gltf = await loader.parseAsync(
    b.buffer.slice(b.byteOffset, b.byteOffset + b.byteLength),
    '',
  );
  const rig = new CharacterRig(gltf, {
    ...config(settings.id),
    height: 1,
    facingDegrees: 0,
    groundOffset: 0,
  ...
  assert.ok(Math.abs(initial.min.y) < 1e-6);
  assert.ok(Math.abs(initial.max.y - 1) < 1e-6);
  const anchor = rig.tracker.getWorldPosition(new THREE.Vector3());
  rig.actor.position.set(1, 0.7, -5);
  rig.actor.rotation.y = 0.8;
  rig.seek(3.5);
  const expected = rig.actor.localToWorld(anchor.clone()),
    actual = rig.tracker.getWorldPosition(new THREE.Vector3());
  assert.ok(Math.hypot(actual.x - expected.x, actual.z - expected.z) < 1e-5);
  rig.seek(rig.player.duration);
  const final = posedBounds(rig.model).clone();
  rig.update(20);
  assert.ok(final.min.distanceTo(posedBounds(rig.model).min) < 1e-6);
  rig.actor.position.set(0, 0, 0);
  rig.actor.rotation.y = 0;
  rig.reset();
  assert.ok(Math.abs(posedBounds(rig.model).min.y) < 1e-6);
});
test('every shipped character config actually resolves against its own GLB', async () => {
  ...
  for (const c of characters.IMABOX_CHARACTERS) {
    ...
    assert.doesNotThrow(
      () => resolveSequence(gltf.animations, settings.sequence, settings.ignore || []),
      `${c.id} (${c.glb})`,
    );
    const rig = new CharacterRig(gltf, settings);
    assert.ok(Math.abs(posedBounds(rig.model).min.y) < 1e-6, `${c.id} starts on the floor`);
    assert.ok(
      Math.abs(posedBounds(rig.model).max.y - settings.height) < 1e-6,
      `${c.id} is the configured height`,
    );
  }
});
test('a stale expectedSHA256 would silently discard the sequence, so it must match the file', async () => {
  ...
  for (const c of characters.IMABOX_CHARACTERS.filter((c) => c.expectedSHA256)) {
    const digest = createHash('sha256')
      .update(fs.readFileSync(new URL('../' + c.glb, import.meta.url)))
      .digest('hex');
...
test('asset checks separate "definitely missing" from "could not tell"', async () => {
  ...
  globalThis.fetch = reply(200, 'model/vnd.usdz+zip');
  assert.equal(await available('x'), 'ok');
  globalThis.fetch = reply(404, 'text/html');
  assert.equal(await available('x'), 'missing');
  globalThis.fetch = reply(200, 'text/html; charset=utf-8');
  assert.equal(await available('x'), 'missing');
  globalThis.fetch = reply(503, 'text/plain');
  assert.equal(await available('x'), 'unknown');
...
test('each page loads the model whose artwork matches its poster', async () => {
  ...
  for (const c of characters.IMABOX_CHARACTERS) {
    ...
    assert.equal(
      c.usdz,
      expected[c.id].replace('.glb', '.usdz'),
      `${c.id}: usdz must match its glb`,
    );
...
test('a duplicate clip can be ignored, but only when named', () => {
  ...
  assert.equal(only[0].duration, 9);
  // Typos and contradictions are caught rather than silently dropping animation.
  assert.throws(
    () => resolveSequence(clips, [{ clips: ['full'] }], ['nope']),
    /not uniquely found/,
  );
  assert.throws(
    () => resolveSequence(clips, [{ clips: ['full', 'tail'] }], ['tail']),
    /both ignored and sequenced/,
  );
});
test('the launch plays once, not twice', async () => {
  ...
  for (const c of characters.IMABOX_CHARACTERS) {
    ...
    const N = 90,
      heights = [];
    for (let i = 0; i < N; i++) {
      rig.seek((rig.player.duration * i) / (N - 1));
      heights.push(posedBounds(rig.model).max.y);
    }
    ...
    for (const h of heights) {
      if (!up && h > floorish) {
        climbs++;
        up = true;
      } else if (up && h < floorish) {
        up = false;
      }
    }
    assert.ok(
      climbs <= 1,
      `${c.id} (${c.glb}) leaves the ground ${climbs} times; the launch is playing more than once`,
    );
  }
});
test('measured opening approaches move once and finish at the stopping distance', () => {
  for (const c of characters.IMABOX_CHARACTERS) {
    ...
    assert.equal(p.at(-1)[1], 1);
    for (let i = 1; i < p.length; i++) {
      assert.ok(p[i][0] > p[i - 1][0]);
      assert.ok(p[i][1] >= p[i - 1][1]);
    }
    assert.ok(approachProgress(0.5, a) > 0, `${c.id} includes opening motion`);
    assert.equal(approachProgress(a.end + 20, a), 1);
    const settings = config(c.id),
      from = { x: 0, z: -settings.startDistance },
      viewer = { x: 0, z: 0 };
    let position = { ...from },
      stopped = false;
    for (let t = 0; t < a.end + 0.1; t += 1 / 60) {
      const candidate = {
        x: 0,
        z:
          -settings.startDistance +
          (settings.startDistance - settings.stopDistance) * approachProgress(t, a),
      };
      if (!stopped) {
        const r = stopBeforeViewer(position, candidate, viewer, settings.stopDistance);
        position.z += (candidate.z - position.z) * r.fraction;
        stopped = r.stopped;
      }
...
test('every USDZ matches its recorded hash and all public routes exist', async () => {
  ...
  for (const c of characters.IMABOX_CHARACTERS) {
    const digest = createHash('sha256')
      .update(fs.readFileSync(new URL('../' + c.usdz, import.meta.url)))
      .digest('hex');
    assert.equal(digest, c.expectedUSDZSHA256);
    const html = fs.readFileSync(
      new URL(`../characters/${c.id}/index.html`, import.meta.url),
      'utf8',
    );
...
test('new exports contain exactly one complete scene timeline', async () => {
  globalThis.self = globalThis;
  const loader = new GLTFLoader().register((p) => {
    p.loadTexture = async () => new THREE.Texture();
    return { name: 'test-textures' };
  });
  for (const c of characters.IMABOX_CHARACTERS) {
    const b = fs.readFileSync(new URL('../' + c.glb, import.meta.url));
    ...
    assert.deepEqual(
      g.animations.map((a) => a.name),
      ['Scene'],
    );
    assert.ok(Math.abs(g.animations[0].duration - c.animationDuration) < 1e-5);
    const rig = new CharacterRig(g, config(c.id));
    rig.seek(rig.player.duration);
    const before = posedBounds(rig.model).clone();
    rig.update(100);
...
test('char5 aura is hidden initially, deforms during its visible interval, and disappears before the ending', async () => {
  ...
  const b = fs.readFileSync(new URL('../char5.glb', import.meta.url));
  const g = await loader.parseAsync(b.buffer.slice(b.byteOffset, b.byteOffset + b.byteLength), '');
  const rig = new CharacterRig(g, config('04')),
    aura = rig.model.getObjectByName('Aura_Baked');
  assert.ok(aura?.isMesh);
  assert.ok(aura.userData.excludeFromGrounding);
  assert.equal(aura.scale.length(), 0);
  rig.seek(75 / 24);
  assert.ok(aura.scale.length() > 0);
  const first = aura.getVertexPosition(100, new THREE.Vector3());
  rig.seek(100 / 24);
  const second = aura.getVertexPosition(100, new THREE.Vector3());
...
test('char3 spins on its head: the head stays planted while the body turns around it', async () => {
  ...
  const b = fs.readFileSync(new URL('../char3.glb', import.meta.url));
  const g = await loader.parseAsync(b.buffer.slice(b.byteOffset, b.byteOffset + b.byteLength), '');
  const rig = new CharacterRig(g, config('03')),
    head = rig.model.getObjectByName('Head'),
    xs = [],
    zs = [];
  assert.equal(rig.tracker.name, 'Armature');
  for (let f = 144; f <= 226; f += 2) {
    rig.seek((f - 1) / 24);
    rig.actor.updateMatrixWorld(true);
    const v = head.getWorldPosition(new THREE.Vector3());
    xs.push(v.x);
    zs.push(v.z);
  }
  assert.ok(Math.max(...xs) - Math.min(...xs) < 0.03, 'head wanders sideways during the spin');
  assert.ok(Math.max(...zs) - Math.min(...zs) < 0.03, 'head wanders forward/back during the spin');
});
test('the page preview has no play control: the animation is for AR only', () => {
  ...
  assert.ok(!/PLAY ANIMATION|preview\.onclick/.test(src));
  const app = fs.readFileSync(new URL('../app.js', import.meta.url), 'utf8');
  assert.ok(!/EXPLORE IN 3D|id="preview"/.test(app), 'no extra preview button on any platform');
});
